...
import { SHARED_ALLOWLIST_ID, SHARED_ALLOWLIST_CAP_ID } from '../constants';

export function AllowlistCard() {
  const { maxFileSize } = useProviderConfig('allowlist', true);
  const [file, setFile] = useState<File | null>(null);
  const [blobId, setBlobId] = useState<string | null>(null);
  const [selectedService, setSelectedService] = useState<string>('service1');
  const [copied, setCopied] = useState(false);

  const { encryptBlob, encrypting, error: encryptError } = useBlobEncryption('allowlist', false);
  const { uploadBlob, uploading, error: uploadError, services } = useBlobUpload(selectedService);
  const { publishBlob, publishing, error: publishError } = useBlobPublish('allowlist');

  const capId = SHARED_ALLOWLIST_CAP_ID;
  ...
  const handleUpload = async () => {
    if (!file || !SHARED_ALLOWLIST_ID || !SHARED_ALLOWLIST_CAP_ID) {
      alert('Please select a file and ensure constants are configured');
      return;
    }

    try {
      const encryptionResult = await encryptBlob(file, SHARED_ALLOWLIST_ID);
      if (!encryptionResult) {
        return;
      }
  ...
  return (
    <Card className="shadow-sm border-border/60">
      <CardHeader>
        ...
        <CardDescription>
          Pay 0.1 SUI once, forever access to any image uploaded. <span className="text-xs font-bold">(UNCHUNKED)</span> 
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {(!SHARED_ALLOWLIST_ID || !SHARED_ALLOWLIST_CAP_ID) && (
          <p className="text-sm text-destructive">
            Please set SHARED_ALLOWLIST_ID and SHARED_ALLOWLIST_CAP_ID in constants.ts
          </p>
        )}
        ...
        <Button
          ...
          disabled={!file || !SHARED_ALLOWLIST_ID || !SHARED_ALLOWLIST_CAP_ID || encrypting || uploading || publishing}
          className="w-full"
        >
