...
export function ChunkConstantCard() {
  const { maxFileSize } = useProviderConfig('chunk_constant_pricing', true);
  ...
  const { encryptBlob, encrypting, error: encryptError } = useBlobEncryption('chunk_constant_pricing', true);
  const { uploadBlob, uploading, error: uploadError, services } = useBlobUpload(selectedService);
  const { publishBlob, publishing, error: publishError } = useBlobPublish('chunk_constant_pricing');
  ...
  const createPolicyObject = async () => {
    ...
    tx.moveCall({
      target: `${packageId}::chunk_constant_pricing::create_chunk_constant_entry`,
      arguments: [
        tx.pure.string('Constant Pricing Blob'),
        tx.pure.u64('10000000'), // 0.01 SUI per chunk
      ],
    });
        ...
        onSuccess: async (result) => {
          try {
            ...
            const policyObject = createdObjects.find((obj: any) =>
              obj.objectType?.includes('chunk_constant_pricing::ChunkConstantPricing')
            );
            const capObject = createdObjects.find((obj: any) =>
              obj.objectType?.includes('chunk_constant_pricing::Cap')
            );
  ...
  const handleUpload = async () => {
    if (!file || !policyObjectId || !capId) {
      alert('Please select a file and create a policy object first');
      return;
    }
  ...
  return (
    <Card className="shadow-sm border-border/60">
      <CardHeader>
        <CardTitle className="text-xl">Fixed Per-Chunk Pricing</CardTitle>
        <CardDescription>
          0.01 SUI per chunk. Auto-creates policy object. <span className="text-xs font-bold">(CHUNKED)</span> 
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <Button
          ...
          disabled={!file || !policyObjectId || !capId || encrypting || uploading || publishing}
          className="w-full"
        >
